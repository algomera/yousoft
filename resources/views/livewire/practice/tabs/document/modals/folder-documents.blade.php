	...
	<x-card class="p-4 space-y-0">
		<div class="space-y-3">
			<div class="flex items-center justify-between">
				...
				@can('upload_required_documents_file')
					<x-button
			...
			@if($current_sub_folder->documents->count() && auth()->user()->role->name === 'technical_asseverator' || auth()->user()->role->name === 'fiscal_asseverator' || auth()->user()->role->name === 'bank')
				<div class="flex justify-end items-center space-x-3">
					@if($current_sub_folder->$status == 1 && auth()->user()->can('approve_required_documents_folder'))
						<x-button wire:click="approve" size="xs">{{ __('Approva') }}</x-button>
					@endif

					@if($current_sub_folder->$status == 2 && auth()->user()->can('disapprove_required_documents_folder'))
						<x-danger-button wire:click="disapprove" size="xs">{{ __('Non Approvare') }}</x-danger-button>
					@endif
				</div>
			@endif
		</div>
						...
						<div class="ml-3 w-full">
							...
							@if($document->type)
								...
								<div class="flex flex-col justify-start">
									<span class="text-gray-500 uppercase text-xs font-semibold">{{ __('Tags') }}:</span>
									<div class="-mx-1">
